    ...
    def test_local_html_references_exist(self):
        for html_file in html_files():
            ...
            for tag, attribute, value in parser.references:
                ...
                with self.subTest(file=html_file.name, tag=tag, attribute=attribute, value=value):
                    target = (html_file.parent / urlparse(value).path).resolve()
                    self.assertTrue(target.is_relative_to(ROOT), f"{value} referenced by {html_file.name} escapes the repo")
                    self.assertTrue(target.is_file(), f"{value} referenced by {html_file.name} is missing")

    def test_products_page_uses_committed_product_images(self):
        parser = StaticReferenceParser()
        parser.feed((ROOT / "products.html").read_text(encoding="utf-8"))

        product_images = [value for tag, attribute, value in parser.references if tag == "img" and attribute == "src"]
